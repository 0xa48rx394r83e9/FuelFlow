...
function centerMap(boundingBox) {
    ...
    if (mapAspectRatio > containerAspectRatio) {
        ...
        translateX = 0;
        translateY = (containerHeight - height * scale) / 2;
    } else {
        scale = containerHeight / height;
        translateX = (containerWidth - width * scale) / 2;
        translateY = 0;
    }
...
function initializeUI() {
    ...
    `;
    document.getElementById("map-container").appendChild(zoomControls);
...
function handleTouchMove(e) {
    ...
    if (e.touches.length === 2) {
        ...
        zoomTowardsPoint(centerX, centerY, scaleFactor * verticalScaleFactor);
    } else if (e.touches.length === 1) {
        drag(e.touches[0]);
    }
}
...
function zoomTowardsPoint(clientX, clientY, scaleFactor) {
