...
let showFuelPrices = true;
let repulsionStrength = 25;
let repulsionFalloff = 2;
let borderForce = 5;
...
function addUIElements() {
    ...
    // Toggle buttons
    const countryToggle = createToggleButton(
        "Toggle Country Names",
        toggleCountryNames
    );
    const priceToggle = createToggleButton(
        "Toggle Fuel Prices",
        toggleFuelPrices
    );
    const centroidToggle = createToggleButton(
        "Toggle Centroids",
        toggleCentroids
    );

    // Sliders
    const repulsionSlider = createSlider(
        "Repulsion Strength",
        0,
        100,
        1,
    ...
    const falloffSlider = createSlider(
        "Repulsion Falloff",
        1,
        5,
        0.1,
    ...
    const borderForceSlider = createSlider(
        "Border Force",
        0,
        20,
        0.1,
    ...
    uiContainer.appendChild(countryToggle);
    uiContainer.appendChild(priceToggle);
    uiContainer.appendChild(centroidToggle);
    uiContainer.appendChild(createLabel("Repulsion Strength"));
    uiContainer.appendChild(repulsionSlider);
    uiContainer.appendChild(createLabel("Repulsion Falloff"));
    uiContainer.appendChild(falloffSlider);
    uiContainer.appendChild(createLabel("Border Force"));
    uiContainer.appendChild(borderForceSlider);
}
...
function evaluateLabelPositioning(labelPositions) {
    ...
    labelPositions.forEach((label, i) => {
        // Check if label is inside its country
    ...
    // Weighted average of scores (you can adjust weights as needed)
...
function optimizeSliderValues() {
    ...
    let bestValues = { repulsionStrength, repulsionFalloff, borderForce };

    for (let i = 0; i < iterations; i++) {
        // Generate random values within reasonable ranges
        const testRepulsionStrength = Math.random() * 50 + 10; // 10 to 60
        const testRepulsionFalloff = Math.random() * 3 + 1; // 1 to 4
        const testBorderForce = Math.random() * 10 + 1; // 1 to 11
        ...
        // Calculate label positions with these values
        ...
        if (score > bestScore) {
            bestScore = score;
            bestValues = { repulsionStrength, repulsionFalloff, borderForce };
        }
    }

    // Apply the best values found
    repulsionStrength = bestValues.repulsionStrength;
    repulsionFalloff = bestValues.repulsionFalloff;
    borderForce = bestValues.borderForce;

    console.log("Optimized values:", bestValues);
}

function updateBorderForce(event) {
    borderForce = parseFloat(event.target.value);
    updateLabels();
}
...
function updateRepulsionFalloff(event) {
    repulsionFalloff = parseFloat(event.target.value);
    updateLabels();
}

function createSlider(name, min, max, step, value, onInput) {
...
function updateLabels() {
    // Remove existing labels and centroids
    svg.querySelectorAll(".country-label, .centroid").forEach((el) =>
        el.remove()
    );

    // Recalculate and render labels
    const labelPositions = calculateLabelPositions();
...
function calculateLabelPositions() {
    ...
    countries.forEach((country) => {
        ...
        let labelHeight = 0;
        if (showCountryNames) labelHeight += 15;
        if (showFuelPrices && fuelPrices[countryName]) labelHeight += 15;
        ...
        labelPositions.push({
            ...
            originalY: cy,
            height: labelHeight,
            country: country, // Store reference to the country path
        });
    });

    // Apply orbital gravitation
    for (let i = 0; i < 50; i++) {
...
function applyGravitation(labelPositions) {
    ...
    labelPositions.forEach((label, i) => {
        ...
        // Calculate force to keep label inside country
        let isInside = false;
        try {
            ...
            // Fallback: assume it's outside if we can't determine
            isInside = false;
        }

        const borderForceFactor = isInside ? 1 : 10; // Stronger force if outside
...
function renderLabels(labelPositions) {
    labelPositions.forEach((label) => {
        ...
        if (showFuelPrices && fuelPrices[label.name]) {
